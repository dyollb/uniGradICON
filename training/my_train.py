...
from unigradicon import make_network, ICONLossExt

from my_dataset import CCDataset, IXIDataset, DRCMRDataset
from my_config import RESULTS_DIR as EXP_DIR


def write_stats(writer, stats: ICONLossExt, ite, prefix=""):
...
BATCH_SIZE = 2 # 4
device_ids = [0]  # [1, 0, 2, 3]
GPUS = len(device_ids)


def get_dataset(request_labels: bool):
    data_num = 25
    datasets =         [
            #CCDataset(
            #    desired_shape=input_shape[2:], device=device_ids[0], data_num=data_num
            #),
            DRCMRDataset(
                desired_shape=input_shape[2:], device=device_ids[0], data_num=data_num, request_labels=request_labels,
            )
        ]
...
def augment(images):
    ...
    for i in range(image_A.shape[0]):
        ...
        for j in range(3):
            ...
            identity[0, j, k] = 1
        identity = identity * (
            torch.randint_like(identity, 0, 2, device=device) * 2 - 1
        )
    ...
    if image_A.shape[1] > 1:
        ...
        warped_A_seg = F.grid_sample(
            image_A[:, 1:], forward_grid, mode="nearest", padding_mode="border"
        )
        warped_A = torch.cat([warped_A, warped_A_seg], axis=1)
    else:
        warped_A = F.grid_sample(image_A, forward_grid, padding_mode="border")
        if have_labels:
            warped_seg_A = F.grid_sample(images[2], forward_grid, mode="nearest", padding_mode="border")
    ...
    if image_B.shape[1] > 1:
        ...
        warped_B_seg = F.grid_sample(
            image_B[:, 1:], forward_grid, mode="nearest", padding_mode="border"
        )
        warped_B = torch.cat([warped_B, warped_B_seg], axis=1)
    else:
        warped_B = F.grid_sample(image_B, forward_grid, padding_mode="border")
        if have_labels:
            warped_seg_B = F.grid_sample(images[3], forward_grid, mode="nearest", padding_mode="border")
...
def train_kernel(optimizer, net, images, writer, ite):
    moving_image, fixed_image = images[0:2]
    moving_seg, fixed_seg = (None, None) if len(images) == 2 else images[2:4]

    optimizer.zero_grad()
    loss_object = net(moving_image, fixed_image, moving_seg, fixed_seg)
    ...
    for epoch in tqdm(range(epochs)):
        for images in data_loader:
            ...
            if data_augmenter is not None:
                with torch.no_grad():
                    images = data_augmenter(images)
            train_kernel(optimizer, net, images, writer, iteration)
            iteration += 1
        ...
        if epoch % eval_period == 0:
            ...
            visualization_moving, visualization_fixed = images[:2]
            visualization_moving, visualization_fixed = (
                visualization_moving[:, :1].cuda(),
                visualization_fixed[:, :1].cuda(),
            )
            unwrapped_net.eval()
            warped = []
            with torch.no_grad():
...
def train_two_stage(
    ...
    if GPUS == 1:
        ...
        net_par = torch.nn.DataParallel(
            net, device_ids=device_ids, output_device=device_ids[0]
        ).cuda()
    ...
    if resume_from != "":
        optimizer.load_state_dict(
            torch.load(
                resume_from.replace("network_weights_", "optimizer_weights_"),
                map_location="cpu",
            )
        )
    ...
    if GPUS == 1:
        ...
        net_2_par = torch.nn.DataParallel(
            net_2, device_ids=device_ids, output_device=device_ids[0]
        ).cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--request_labels", required=False, type=bool, default=False)
    args = parser.parse_args()
    resume_from = args.resume_from
    request_labels = args.request_labels

    import footsteps

    footsteps.initialize(run_name=args.name, output_root=EXP_DIR)

    dataset = get_dataset(request_labels)
    ...
    train_two_stage(
        input_shape, dataloader, val_dataloader, GPUS, [801, 201], 20, 20, resume_from
    )
